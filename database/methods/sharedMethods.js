...
function checkPassword(req, res, next) {
  ...
  if (!req.body.userName){
    res.status(400).send({ error: 'Username/Password required' });
    return;
  }
  ...
  .exec((err, dev) => {
    ...
    dev.comparePassword(req.body.password, (error, isMatch) => {
...
function checkDevID(req,res,next) {
  ...
  Models.Dev.findById(_id, (err, dev) => {
    if (!dev) {
      res.sendStatus(404);
    } else {
      req.body.dev = dev;
...
function openDB(req, res, next) {
  ...
  Models.DB.findOne({ name: dbName, 'collections.name': colName }, { "collections.$": 1 }, (err, result) => {
