...
function checkJwt(req, res, next) {
  if (!req.headers.authorization) {
    ...
    if (tokenHeader === 'Bearer') {
      ...
      jwt.verify(token, 'sheep host', (err, decoded) => {
        if (err) {
...
function keyCheck(req, res, next) {
  if (res.locals.token) {
    ...
    Models.Dev.findOne(query, {}, (err, dev) => {
      if (dev === null) {
        ...
        res.locals.apikey.master = permission;
        next();
      } else {
        res.json({ error: 'Not Valid' });
      }
...
function updatePermissions(req, res) {
  if (res.locals.apikey.master) {
    ...
    Models.Dev.findOneAndUpdate(query, { $set: { 'api.clientPermissions': client } }, { new: true }, (err) => {
